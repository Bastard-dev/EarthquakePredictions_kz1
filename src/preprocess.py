...
#     df.to_csv("artifacts/preprocess/enrich_administrative_areas.csv", sep="\t", index=False)
    ...
    for i in range(1, 6):
        for n in ["mag", "depth"]:
            for m in agg_funcs:
                train_cj['{}_{}_{}ylag'.format(n, m, i)] = train_cj.groupby(["REGION_"])[n+"_"+m].apply(lambda v: v.shift(i)).reset_index(level=0, drop=True)

    for n in ["mag", "depth"]:
        for m in agg_funcs:
            for LAG in roll_years_agg:
                train_cj['rollsum_{}y_{}_{}'.format(LAG, n, m)] = train_cj.groupby(["REGION_"])[n+"_"+m].apply(lambda v: v.shift(1).rolling(LAG).sum()).reset_index(level=0, drop=True)
                train_cj['rollmin_{}y_{}_{}'.format(LAG, n, m)] = train_cj.groupby(["REGION_"])[n+"_"+m].apply(lambda v: v.shift(1).rolling(LAG).min()).reset_index(level=0, drop=True)
                train_cj['rollmax_{}y_{}_{}'.format(LAG, n, m)] = train_cj.groupby(["REGION_"])[n+"_"+m].apply(lambda v: v.shift(1).rolling(LAG).max()).reset_index(level=0, drop=True)
                train_cj['rollmean_{}y_{}_{}'.format(LAG, n, m)] = train_cj.groupby(["REGION_"])[n+"_"+m].apply(lambda v: v.shift(1).rolling(LAG).mean()).reset_index(level=0, drop=True)
